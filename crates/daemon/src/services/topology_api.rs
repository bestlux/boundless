...
impl TopologyService for TopologyApi {
    async fn list_peers(&self, request: Request<Empty>) -> Result<Response<PeerListReply>, Status> {
        control_plane_api(self.0.clone()).list_peers(request).await
    }
    ...
    async fn remove_peer(
        ...
        control_plane_api(self.0.clone()).remove_peer(request).await
    }

    async fn layout_show(&self, request: Request<Empty>) -> Result<Response<LayoutReply>, Status> {
        control_plane_api(self.0.clone()).layout_show(request).await
    }
    ...
    async fn layout_set(
        ...
        control_plane_api(self.0.clone()).layout_set(request).await
    }
}
