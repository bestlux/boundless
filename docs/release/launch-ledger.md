...
| 7 | Clipboard share works in service mode | open | Confirmed broken 2026-07-07 despite `share_clipboard=true` both sides. The clipboard runtime (`crates/daemon/src/clipboard.rs`) polls `WindowsClipboardBackend` inside the LocalSystem daemon in session 0, whose window-station clipboard is isolated from the user desktop — local copies are never seen, remote payloads are applied to an invisible clipboard. Input works because it is brokered through the tray (`crates/tray/src/input_broker.rs`); clipboard needs the same user-session broker routing (extend attach/exchange with clipboard read/apply ops). Service-mode clipboard has never worked and cannot without this. |
