...
| Use service | Install service to control elevated apps/lock screen. | Optional Windows service mode with explicit status and IPC ACLs. BND-NEXT-44 targets ordinary elevated apps through a minimal signed Program Files injector after a UIAccess versus explicit-elevation policy proof, while capture and clipboard remain unelevated; UAC secure desktop and lock-screen support remain separate, unclaimed work. | cli-ready | `boundless-service`, user-session input injector, `platform-windows`, installer | `service install --binary <admin-protected path>`, `service status`, `service start`, `service stop` | future Settings service panel | control-pipe ACL tests; BND-NEXT-44 signature/path/token and elevated-window smoke; UAC secure-desktop and lock-screen classification still required | yes |
