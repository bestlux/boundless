...
| Use service | Install service to control elevated apps/lock screen. | Optional Windows service mode with explicit status, IPC ACLs, install/uninstall, elevated-app validation, and lock-screen claim classification. | cli-ready | `boundless-service`, `platform-windows`, installer | `service install --binary <admin-protected path>`, `service status`, `service start`, `service stop` | future Settings service panel | control-pipe ACL tests; service host/CLI compile validation; installer includes service binary and closes service process; elevated service smoke still required for release signoff; elevated-app and lock-screen validation still required | yes |
