...
The planned v5.0.15 one-user dogfood exception does not change that rule. An explicitly enabled, MSI-owned, unsigned `requireAdministrator` injector may be evaluated with an **Unknown Publisher** UAC prompt and `unsigned dogfood` status for the same split-token administrator only. That experiment is not UIAccess, trusted-publisher, secure-desktop, lock-screen, alternate-admin, or validated parity evidence.
...
| Use service | Install service to control elevated apps/lock screen. | Optional Windows service mode with explicit status and IPC ACLs. BND-NEXT-44 targets ordinary same-user elevated apps through a minimal Program Files injector while capture and clipboard remain unelevated. One-user dogfood may explicitly enable an unsigned `requireAdministrator` fallback with an **Unknown Publisher** UAC prompt and `unsigned dogfood` status; UIAccess and polished/trusted-publisher support require trusted signing. UAC secure desktop, lock screen, Winlogon, other sessions, and alternate-admin credentials remain separate and unclaimed. | cli-ready | `boundless-service`, user-session input injector, `platform-windows`, installer | `service install --binary <admin-protected path>`, `service status`, `service start`, `service stop` | future explicit enable/disable action, signing status, and capability reason; no automatic UAC | control-pipe ACL tests; canonical MSI path and component ownership; actual PID/token/SID/session/handshake checks; one-tray/at-most-one-injector lifecycle; unsigned dogfood classification and Unknown Publisher consent; normal/elevated-window smoke; trusted signature and `TokenUIAccess=1` before UIAccess claims; UAC secure-desktop and lock-screen classification still required | yes |
| Uninstall service | Remove service from computer. | Service uninstall path removes service, stops daemon, and preserves or migrates user config safely. | cli-ready | `boundless-service`, installer/service host | `service uninstall` | future Settings service panel | installer smoke asserts no registered Boundless service remains after uninstall; `scripts/dev/service-smoke.ps1` covers service uninstall when run elevated | yes |
...
| Local control endpoint security | Not an MWB user setting; MWB service/control behavior is local to Windows. | Current-user named-pipe ACLs, service-to-user privilege separation, localhost TCP fallback warnings, and tests that unauthorized local users cannot invoke privileged actions. | cli-ready | `platform-windows`, `adapter-ipc-grpc`, `daemon` control plane | `daemon status`, `service status`, future diagnostics checks | Settings service/control health | current-user/service pipe ACL construction tests; elevated service smoke; unauthorized-user denial and localhost fallback warning tests still required | yes |
...
| Add firewall rule | Install firewall rule for MWB. | Optional firewall rule install/check/remove with admin prompts and diagnostics, not silent mutation. | not-started | Windows installer/scripts/platform glue | future firewall command | future Settings action | firewall rule validation; no silent installer mutation in v5 package hardening | yes |
